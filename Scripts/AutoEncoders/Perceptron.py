...
class Autoencoder(Model):
	def __init__(self, latent_dim, pred_shape, pred_channels):
		self.name = "perceptron_LD{}_H{}_W{}_C{}".format(latent_dim, *pred_shape, pred_channels)
		...
		self.decoder = K.Sequential([
			Dense(pred_shape[0]*pred_shape[0]*pred_channels, activation='sigmoid'),
			Reshape((*pred_shape, pred_channels))
			])
...
if __name__ == '__main__':
	...
	#prepare the network
	autoencoder = Autoencoder(latent_dim, pred_shape, color_channels)
